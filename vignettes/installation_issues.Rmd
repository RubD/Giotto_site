...
*Assuming [Homebrew](https://brew.sh/) installed and R installed via Homebrew: `brew install --cask r`*
...
New release of Apple Silicon Macs requires a few extra steps otherwise
installing packages with building will throw errors. 

This is specific to Apple silicon Macs, an experimental build of 
GNU Fortran compiler is required otherwise errors like these
would pop up during installation:

```error
ld: warning: directory not found for option '-L/opt/R/arm64/gfortran/lib/gcc/aarch64-apple-darwin20.6.0/12.0.1'
```

This is due to [no Fortran compiler](https://support.bioconductor.org/p/9138121/)
and it can be solved with a experimental build of Fortran complier.
...
openMP, gcc can be installed. homebrew would be the
...
Changing the sym-linked to BLAS lib would improve significantly in 
matrix calculation, [up to ~30x Fold in some cases](https://www.mail-archive.com/r-sig-mac@r-project.org/msg06199.html).
